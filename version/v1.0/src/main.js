...
ipcMain.handle('v2:library:list', async () => {
  try {
    ...
    const libs = (data.libraries || []).map((l) => {
      const p = l && typeof l.path === 'string' ? l.path : '';
      let invalidReason = null;
      try {
        if (!p || !fs.existsSync(p)) invalidReason = '路径不存在';
        else {
          const st = fs.statSync(p);
          if (!st.isDirectory()) invalidReason = '不是文件夹';
        }
      } catch (e) {
        invalidReason = e.message || '路径不可访问';
      }
      if (invalidReason) return { ...l, invalidReason };
      const copy = { ...l };
      if ('invalidReason' in copy) delete copy.invalidReason;
      return copy;
    });
    return { success: true, data: { ...data, libraries: libs, settings } };
  } catch (e) {
    return { success: false, error: e.message };
  }
});
...
ipcMain.handle('v2:library:relink', async (event, req) => {
  try {
    if (!mainWindow) return { success: false, error: 'no-window' };
    const libraryId = req && typeof req.libraryId === 'string' ? req.libraryId : null;
    if (!libraryId) return { success: false, error: 'bad-request' };
